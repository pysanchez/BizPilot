...
@app.get("/api/ingresos/{id_empresa}")
def listar_ingresos_y_cobros(id_empresa: int):
    datos = database.obtener_ingresos_y_cobros(id_empresa)

    if datos is None:
        return {
            "status": "error",
            "mensaje": "No se pudieron consultar los ingresos y cobros",
            "data": []
        }
...
class GastoOperativoSchema(BaseModel):
    id_empresa: int = Field(gt=0)
    ...
    @classmethod
    def limpiar_textos_gasto(cls, valor):
        if valor is None or str(valor).strip() == "":
            return None
...
@app.post("/api/gastos")
def crear_gasto(gasto: GastoOperativoSchema):
    return database.registrar_gasto_operativo(
...
def anular_gasto(
    id_gasto: int,
    datos: AnularGastoSchema
):
    return database.anular_gasto_operativo(
